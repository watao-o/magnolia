<template>
  ...
  >
    ...
    <v-row>
      <v-col cols="2" class="border custom-col"
        ><v-icon icon="mdi-human" /><span class="custom-text"
          >Player</span
        ></v-col
      >
      <v-col cols="2" class="border custom-col"
        ><v-icon icon="mdi-gold" /><span class="custom-text">お金</span></v-col
      >
      <v-col cols="2" class="border custom-col"
        ><v-icon icon="mdi-shield-cross" /><span class="custom-text"
          >戦力</span
        ></v-col
      >
      <v-col cols="1" class="border custom-col"
        ><v-icon icon="mdi-cog-box" /><span class="custom-text"
          >技術点</span
        ></v-col
      >
      <v-col cols="1" class="border custom-col"
      ...
      >
      <v-col cols="1" class="border custom-col"
        ><v-icon icon="mdi-book-cross" /><span class="custom-text"
          >信仰点</span
        ></v-col
      >
      <v-col cols="1" class="border custom-col"
      ...
      >
      <v-col cols="2" class="border custom-col"
        ><v-icon icon="mdi-star-face" /><span class="custom-text"
          >VP</span
        ></v-col
      >
    </v-row>
      ...
      >
        <v-row>
          ...
          <v-col cols="1" class="border player-back">
            <span class="custom-text"> {{ user.status.tech }} 点 </span>
          </v-col>
          <v-col cols="1" class="border player-back">
            <span class="custom-text">
          ...
          <v-col cols="1" class="border player-back">
            <span class="custom-text"> {{ user.status.faith }} 点 </span>
          </v-col>
          <v-col cols="1" class="border player-back">
            <span class="custom-text">
    ...
    <v-row class="pt-6">
      <v-col cols="6" class="custom-col border"
        ><v-icon icon="mdi-cards" /><span class="custom-text"
          >プレイヤーカード置き場</span
        ></v-col
      >
      <v-col cols="6" class="custom-col border"
        ><v-icon icon="mdi-cards" /><span class="custom-text">手札</span></v-col
      >
    </v-row>
    ...
    <v-list class="pa-0 pt-6 transparent-list">
      <v-list-item
        v-for="(user, index) in room.users
          ? room.users.filter((u) => u.name !== userName)
          : []"
        :key="index"
        class="pa-0"
      >
        <v-row class="py-2"
          ><span class="custom-text">{{ user.name }}</span></v-row
        >
        <v-row>
          <v-col cols="6">
            <other-card-place
    ...
    <!-- ゲーム終了結果通知 -->
    <result-dialog ref="resultDialog" :room="room" />
  </v-container>
</template>
...
export default {
  ...
  computed: {
    otherPlayers () {
      ...
      return this.room.users
        ? this.room.users.filter((u) => u.name !== this.userName)
        : []
    }
  },
  watch: {
  ...
  methods: {
    ...
    putPhase () {
      // ダイアログ表示
      this.$refs.putPhaseDialog.openDialog(
        'ドローフェーズです\r\nカードを捨てて設置してください'
      )
      this.trashFlg = true
    },
    ...
    addExistCardList (selectedCard) {
      ...
      unitInfo = {
        ...unitInfo,
        sesrialNum: selectedCard.serialNum,
        ...
        switch (skill.effectTarget) {
          ...
          case TARGET.VP:
            ...
            if (skill.synergyTarget === SYNERGY.FAITH_LEVEL) {
              this.status.vp +=
                skill.effectValue * this._getLevel(this.status.faith)
              // 種族バフ
            } else {
              // 種族が一致するカードを取得
              const sameKindCards = this.existCardList.filter(
                (card) => card.kind === skill.synergyTarget
              )
    ...
    _getFrontUnits () {
      ...
      // 横位置ごとに分類する
      for (let i = 0; i < 5; i++) {
        const verticalCards = this.existCardList.filter(
          (card) => card.posX === i
        )
    ...
    elvenArcher (frontUits) {
      // エルフの射手を設置しているか確認
      const elvenArchers = this.existCardList.filter(
        (ec) => ec.unitId === ELVEN_ARCHER_ID
      )
      if (!_.isEmpty(elvenArchers)) {
        // 前線に存在しているか確認
        const frontElevenArdhers = frontUits.filter(
          (fu) => fu.unitId === ELVEN_ARCHER_ID
        )
        // エルフの射手の戦力を取得
        const force = this.cardData.unitList.find(
          (u) => u.unitId === ELVEN_ARCHER_ID
        ).force
        // 戦力 × (配置している数 - 前線にいる数)
        this.status.force +=
          force * (elvenArchers.length - frontElevenArdhers.length)
      }
    },
    /**
     * 戦争勝利点計算
     */
    getWarVp (rank) {
      return WAR_VP.find((wv) => wv.player === this.playerNum).result.find(
        (r) => r.rank === rank
      ).vp
    },
    /**
